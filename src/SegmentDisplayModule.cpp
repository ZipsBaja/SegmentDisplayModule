...
namespace uazips
{
        ...
        : CollectionModule(), device_count(1), digit_count(device_count * digits_per_device)
    {
        ...
        dev->upsidedown = settings.is_upside_down;
    ...
    SegmentDisplayModule::SegmentDisplayModule(SegmentDisplaySettings* settings, size_t display_amount)
        : CollectionModule(), device_count(display_amount), digit_count(display_amount * digits_per_device)
    {
        for (size_t i = 0; i < display_amount; i++)
        {
            ...
            dev->upsidedown = settings[i].is_upside_down;
    ...
    {
        int digits_left = std::min(digit_count, strlen(text));
    ...
    void SegmentDisplayModule::DisplayAnimationAll(const ArrayView2D<uint32_t>& frames, float target_fps, const bool* loop_until, bool reverse)
    {
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimationAll(const ArrayView<uint32_t>& frames, float target_fps, const bool* loop_until, bool reverse)
    {
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimationAll(const uint32_t** frames, size_t frame_count, size_t display_count, float target_fps, const bool* loop_until, bool reverse)
    {
        display_count = display_count > device_count ? device_count : display_count;
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimationAll(const uint32_t* frames, size_t frame_count, float target_fps, const bool* loop_until, bool reverse)
    {
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimation(size_t index, const ArrayView<uint32_t>& frames, float target_fps, const bool* loop_until, bool reverse)
    {
        index = index >= device_count ? device_count - 1 : index;
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimation(const SegmentDisplaySettings& device, const ArrayView<uint32_t>& frames, float target_fps, const bool* loop_until, bool reverse)
    {
        DisplayAnimation(FindElement(device.device_ptr), frames, target_fps, loop_until, reverse);
    }

    void SegmentDisplayModule::DisplayAnimation(size_t index, const uint32_t* frames, size_t frame_count, float target_fps, const bool* loop_until, bool reverse)
    {
        index = index >= device_count ? device_count - 1 : index;
        uint64_t us_interval = static_cast<uint64_t>(1.0f / target_fps * 1e6f);
        uint64_t cumulative_us = 0;
    ...
    void SegmentDisplayModule::DisplayAnimation(const SegmentDisplaySettings& device, const uint32_t* frames, size_t frame_count, float target_fps, const bool* loop_until, bool reverse)
    {
        DisplayAnimation(FindElement(device.device_ptr), frames, frame_count, target_fps, loop_until, reverse);
    }
    ...
    void SegmentDisplayModule::DisplayAnimationOnceAll(const uint32_t** frames, size_t frame_count, size_t display_count, float target_fps, bool keep_last_frame, bool reverse)
    {
        ...
        for (size_t current_frame = reverse ? frame_count - 1 : 0; reverse ? current_frame >= 0 : current_frame < frame_count; reverse? current_frame-- : current_frame++)
        {
            DisplaySegmentsAll(frames[current_frame], frame_count);
    ...
    {
        DisplayAnimationOnce(FindElement(device.device_ptr), frames, target_fps, keep_last_frame, reverse);
    }
    ...
    {
        DisplayAnimationOnce(FindElement(device.device_ptr), frames, frame_count, target_fps, keep_last_frame, reverse);
    }
